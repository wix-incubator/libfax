...
class SendCharFaxResponseParserTest extends SpecificationWithJUnit {
  trait Ctx extends Scope {
    ...
    def beSendCharFaxResponse(value: Matcher[JLong] = AlwaysMatcher()): Matcher[SendCharFaxResponse] = {
      value ^^ { (_: SendCharFaxResponse).value aka "value" }
    }
  }
  ...
  "stringify and then parse" should {
    "yield an object similar to the original one" in new Ctx {
      ...
      parser.parse(xml) must beSendCharFaxResponse(
        value = ===(someResponse.value)
      )
    }
  }
}
